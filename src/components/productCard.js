...
const ProductCard = ({ vendorID, vendorName, vendorLocation, coverPhoto, cardScrolls, ratingSection, rating, vendorCountry }) => {
  return (
    <Link to={`/tailor-profile/${btoa(vendorID)}`} className="grid gap-2">
      ...
      >
        ...
        <div className="absolute inset-0 flex justify-end items-end p-2 hover:bg-black/40 transition-all cursor-pointer">
          <div className={`${ratingSection === false ? "hidden" : "p-2 rounded-md bg-black/5 backdrop-blur-md flex items-center gap-1 text-white"}`}>
      ...
      <div className="truncate">
        ...
        <div className="text-[13px] text-gray-400 truncate">{vendorLocation}, {vendorCountry}</div>
      </div>
    </Link>
  );
};
