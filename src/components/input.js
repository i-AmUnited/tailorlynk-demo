...
const Input = ({label,type, placeholder, value, name, readOnly, disabled, onChange, onBlur, customStyles, onError, isPassword}) => {
  ...
  return (
    <div>
          <div className="grid gap-[2px] text-sm">
            ...
            {isPassword === "true" ?
              ...
              <div><input type={inputType} readOnly={readOnly} disabled={disabled} onChange={onChange} placeholder={placeholder} value={value} name={name} onBlur={onBlur} className={`w-full px-4 py-5 md:py-4 border text-sm focus:outline focus:outline-primary rounded ${customStyles}`}/></div>
            </div>
          </div>
