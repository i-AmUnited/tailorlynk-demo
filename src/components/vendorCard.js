    ...
    // <Link to={`/tailor-profile/${btoa(vendorID)}`} className="grid gap-2">
      ...
      <div className="grid gap-2 w-24 h-32 pt-2 flex-shrink-0">
        <div className="flex justify-center">
          {/* <img src={coverPhoto === null ? placeholderImage : coverPhoto} alt="brand_image" className="size-14 rounded-full object-cover outline outline-[4px] outline-primary/20"/> */}
          <img 
            src={coverPhoto}
            onError={(e) => {
            e.target.src = imgFallback; // Set fallback image on error
            }} alt="brand_image" className="size-14 rounded-full object-cover outline outline-[4px] outline-primary/20"/>
        </div>
