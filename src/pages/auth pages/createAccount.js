...
import { completeUserRegistration, userAccountRegistration, verifyUserEmail } from "../../hooks/local/reducer";
...
const CreateAccount = () => {
  ...
  const completeregistrationForm = useFormik({
    ...
    onSubmit: async (values) => {
      const { email, fullName, phoneNumber, password, confirmPassword } = values;
      let registrationData = { email_address: email, full_name: fullName, phone_number: phoneNumber, password, confirm_password: confirmPassword};
      const { payload } = await dispatch(completeUserRegistration(registrationData));
      ...
      <div className="w-full md:w-[50%] lg:w-[35%] grid gap-4">
        ...
        <div className="bg-white rounded-lg border px-5 py-14">
          ...
          {provideEmail && 
            <form onSubmit={provideEmailForm.handleSubmit}>
              <div className="flex justify-between items-center mb-10">
                ...
                <Link to={"/sign-in"} className="underline underline-offset-2 text-brandGreen text-xs">
                  Sign in
                </Link>
              </div>
          ...
          <form onSubmit={completeregistrationForm.handleSubmit}>
            ...
            <div className="grid gap-4">
              <Input label={"Email address:"} name={"email"} value={completeregistrationForm.values.email} disabled={true} />
              <Input
                label={"Full name:"}
                name={"fullName"}
                ...
                onError={
                completeregistrationForm.touched.fullName && completeregistrationForm.errors.fullName ? completeregistrationForm.errors.fullName : null}
                />
                <Input
                label={"Phone number:"}
                name={"phoneNumber"}
                ...
                onBlur={completeregistrationForm.handleBlur}
                onError={
                completeregistrationForm.touched.phoneNumber && completeregistrationForm.errors.phoneNumber ? completeregistrationForm.errors.phoneNumber : null}
                />
              <Input
                ...
                value={completeregistrationForm.values.password}
                  ...
                  onBlur={completeregistrationForm.handleBlur}
                  onError={
                  completeregistrationForm.touched.password && completeregistrationForm.errors.password ? completeregistrationForm.errors.password : null}
                 />
              <Input
                ...
                value={completeregistrationForm.values.confirmPassword}
                  ...
                  onBlur={completeregistrationForm.handleBlur}
                  onError={
                  completeregistrationForm.touched.confirmPassword && completeregistrationForm.errors.confirmPassword ? completeregistrationForm.errors.confirmPassword : null}
                />
            </div>
            <div className="mt-6 flex justify-center">
              <Button
