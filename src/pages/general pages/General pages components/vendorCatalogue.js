        ...
        {currentProducts.map((product) => (
          <Link key={product.id} to={`/product-detail/${btoa(product.catalogueId || product.materialId)}`}>
            <div className="grid content-between md:text-xs">
              <LazyLoadImage effect="blur" src={product.styleImageOne || product.materialImageOne} alt={product.name} className="w-full aspect-video md:aspect-square object-cover rounded-md" />
              <div className="line-clamp-none md:line-clamp-1 mt-2 mb-1">{product.styleName || product.materialName}</div>
              <div className="text-black/50 font-semibold">{product.cost || product.price || product.costPerYard}</div>
            </div>
          </Link>
        ))}
      </div>
